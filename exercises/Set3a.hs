...
firstHalf' x 0 = ""
firstHalf' [] _ = []
firstHalf' x i = (head x): firstHalf' (tail x) (i-1) 

firstHalf xs = 
    if odd (length xs)
        then firstHalf' xs ((length xs) `div` 2 + 1)
    else
        firstHalf' xs ((length xs) `div` 2)
...
capitalize sentence = unwords [capitalizeFirst x | x<-words sentence]
